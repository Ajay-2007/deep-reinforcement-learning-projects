...
if __name__ == "__main__":
    ...
    pharse_pairs, emb_dict = data.load_data(args.data)
    log.info("Obtained %d phrase pairs with %d uniq words", len(pharse_pairs), len(emb_dict))
    train_data = data.encode_phrase_pairs(pharse_pairs, emb_dict)
    ...
    log.info("Processed %d phrase, mean BLEU = %.4f", seq_count, sum_bleu / seq_count)
