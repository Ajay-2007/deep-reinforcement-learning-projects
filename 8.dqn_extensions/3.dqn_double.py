...
def calc_loss_double_dqn(batch, net, tgt_net, gamma, device="cpu", double=True):
    ...
    with torch.no_grad():
        ...
        exp_sa_vals = rewards_v + next_state_vals.detach() * gamma
...
if __name__ == "__main__":
    ...
    def process_batch(engine, batch):
        optimizer.zero_grad()
        loss_v = calc_loss_double_dqn(batch, net, tgt_net.target_model, gamma=params.gamma, device=device, double=args.double)
        loss_v.backward()
        optimizer.step()
