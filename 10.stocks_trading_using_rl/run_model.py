...
if __name__ == "__main__":
    ...
    parser.add_argument("--commision", type=float, default=0.1, help="Commission size in percent, default=0.1")
    ...
    env = environ.StocksEnv({"TEST": prices}, bars_count=args.bars, reset_on_close=False, commission=args.commission,
                            state_id=args.conv, random_ofs_on_reset=False, reward_on_close=False, volumes=False)

    if args.conv:
        net = model.DQNConv1D(env.observation_space.shape, env.action_space.n)
    else:
        net = model.SimpleFFDQN(env.observation_space.shape, env.action_space.n)
